...
import {
  buildLocalCardContent,
  buildLocalMedalContent,
  extractLocalOres,
} from './localClassification.js';
...
if (apiKey) {
  ...
  console.warn('OpenAI API key not configured. Using local classification responses.');
}

export async function extractOres(userInput) {
  if (!openai) {
    return extractLocalOres(userInput);
  }
...
- Wisdom：学习、思考、研究类成就
- Will：健身、习惯、坚持类成就
- Creation：创作、设计、编码类成就
- Connection：社交、合作、沟通类成就
...
export async function refineOres(ores) {
  const oreDescriptions = ores.map((ore) => ore.text || ore.refined_data?.text).join('；');

  if (!openai) {
    return buildLocalCardContent(ores);
  }
...
2. 一段 DALL-E 生图 prompt（英文，描述一张魔法卡片的插画）
3. 一段简短的成就描述（40字以内）
...
1. 一个神圣的勋章称号（8字以内）
2. 一段史诗般的成就描述（50字以内）
3. 一段 DALL-E 生图 prompt（英文，描述一枚发光的几何勋章）
